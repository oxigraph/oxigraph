...
def plot_usecase(name: str):
    ...
    for file in glob(f"bsbm.{name}.*.xml"):
        parts = file.split(".")
        run = ".".join(parts[2:-1])
        for query in ET.parse(file).getroot().find("queries").findall("query"):
            ...
            for child in query.iter():
                if child.tag == "aqet":
                    ...
                    avgresults_by_query[query_id][int(parts[-3])][run] = float(
                        query.find("avgresults").text
                    )
    plot_y_per_x_per_plot(aqet, "query id", "execution time (s)", f"bsbm.{name}.svg")
