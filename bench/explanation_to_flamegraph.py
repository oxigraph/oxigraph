...
def add_to_trace(node, path):
    ...
    samples = node["duration in seconds"] - sum(
        child["duration in seconds"] for child in node.get("children", ())
    )
...
if inferno:
    args.flamegraph_svg.write_text(
        subprocess.run(
            [inferno], input="\n".join(trace), stdout=subprocess.PIPE, text=True
        ).stdout
    )
elif flamegraph_pl:
    with NamedTemporaryFile("w+t") as fp:
        ...
        args.flamegraph_svg.write_text(
            subprocess.run(
                [flamegraph_pl, fp.name], stdout=subprocess.PIPE, text=True
            ).stdout
        )
else:
    raise Exception(
