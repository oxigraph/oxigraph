...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let rdf_uri = |suffix: &str| -> Result<NamedNode, Box<dyn Error>> {
        Ok(NamedNode::new(format!("http://www.w3.org/1999/02/22-rdf-syntax-ns#{}", suffix))?)
    };
    
    let rdfs_uri = |suffix: &str| -> Result<NamedNode, Box<dyn Error>> {
        Ok(NamedNode::new(format!("http://www.w3.org/2000/01/rdf-schema#{}", suffix))?)
    };
    ...
    store.insert(&Quad::new(
        ex_uri("author1")?,
        ex_uri("born")?,
        Literal::new_typed_literal("1892-01-03", NamedNode::new("http://www.w3.org/2001/XMLSchema#date")?),
        GraphName::DefaultGraph,
    ))?;
    ...
    let (results, explanation) = store.explain_query_opt(
        query,
