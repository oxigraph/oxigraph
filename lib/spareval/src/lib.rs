...
pub use crate::dataset::{ExpressionTerm, InternalQuad, QueryDataset};
...
/// You must select the proper dataset before calling this function.
///
/// To adapt this function to work on your own RDF dataset, you need to implement the [`QueryDataset`] trait.
///
/// ```
        ...
        let (results, plan_node_with_stats, planning_duration) = match query {
            Query::Select {
                ...
                let (results, explanation) = SimpleEvaluator::new(
                    dataset,
                    base_iri.map(Arc::new),
                    Arc::new(self.service_handler),
                    Arc::new(self.custom_functions),
                    self.run_stats,
                )
