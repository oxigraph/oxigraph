...
use crate::dataset::{ExpressionTerm, InternalQuad, QueryDataset};
...
/// Wrapper on top of [`QueryDataset`]
struct EvalDataset<D: QueryDataset> {
    dataset: Arc<D>,
}

impl<D: QueryDataset> EvalDataset<D> {
...
impl<D: QueryDataset> Clone for EvalDataset<D> {
    #[inline]
    fn clone(&self) -> Self {
        Self {
...
pub struct InternalTuple<D: QueryDataset> {
    inner: Vec<Option<D::InternalTerm>>,
}

impl<D: QueryDataset> InternalTuple<D> {
...
impl<D: QueryDataset> Clone for InternalTuple<D> {
    fn clone(&self) -> Self {
        Self {
...
impl<D: QueryDataset> PartialEq for InternalTuple<D> {
    #[inline]
...
impl<D: QueryDataset> Eq for InternalTuple<D> {}

impl<D: QueryDataset> Hash for InternalTuple<D> {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.inner.hash(state)
    }
}

impl<D: QueryDataset> IntoIterator for InternalTuple<D> {
...
pub struct SimpleEvaluator<D: QueryDataset> {
    dataset: EvalDataset<D>,
    base_iri: Option<Arc<Iri<String>>>,
    now: DateTime,
    service_handler: Arc<ServiceHandlerRegistry>,
    custom_functions: Arc<CustomFunctionRegistry>,
    run_stats: bool,
}

impl<D: QueryDataset> SimpleEvaluator<D> {
    pub fn new(
        dataset: D,
        base_iri: Option<Arc<Iri<String>>>,
        service_handler: Arc<ServiceHandlerRegistry>,
    ...
    ) -> (QuerySolutionIter, Arc<EvalNodeWithStats>) {
    ...
    pub fn evaluate_ask(
        &self,
        pattern: &GraphPattern,
    ) -> (Result<bool, QueryEvaluationError>, Arc<EvalNodeWithStats>) {
        let mut variables = Vec::new();
        let (eval, stats) = self.graph_pattern_evaluator(pattern, &mut variables);
        let from = InternalTuple::with_capacity(variables.len());
    ...
    ) -> (QueryTripleIter, Arc<EvalNodeWithStats>) {
        let mut variables = Vec::new();
        let (eval, stats) = self.graph_pattern_evaluator(pattern, &mut variables);
    ...
    pub fn graph_pattern_evaluator(
        ...
        Arc<dyn Fn(InternalTuple<D>) -> InternalTuplesIterator<D>>,
        Arc<EvalNodeWithStats>,
    ) {
        ...
        let stats = Arc::new(EvalNodeWithStats {
            ...
            exec_duration: Mutex::new(self.run_stats.then(DayTimeDuration::default)),
        });
        if self.run_stats {
            let stats = Arc::clone(&stats);
            evaluator = Arc::new(move |tuple| {
                let start = Timer::now();
                let inner = evaluator(tuple);
                {
                    let mut exec_duration = stats.exec_duration.lock().unwrap();
                    *exec_duration = exec_duration.and_then(|d| d.checked_add(start.elapsed()?));
                }
    ...
    fn build_graph_pattern_evaluator(
        ...
        match pattern {
            GraphPattern::Values {
                ...
                {
                    Ok(encoded_tuples) => Arc::new(move |from| {
                        Box::new(
                            encoded_tuples
                                .iter()
            ...
            GraphPattern::Lateral { left, right } => {
                ...
                } = right.as_ref()
                {
                    if nested_left.is_empty_singleton() {
                        ...
                        stat_children.push(right_stats);
                        return Arc::new(move |from| {
                            Box::new(ForLoopLeftJoinIterator {
                                right_evaluator: Arc::clone(&right),
                                left_iter: left(from),
            ...
            GraphPattern::Distinct { inner } => {
                let (child, child_stats) = self.graph_pattern_evaluator(inner, encoded_variables);
                stat_children.push(child_stats);
                Arc::new(move |from| Box::new(hash_deduplicate(child(from))))
            }
            ...
            GraphPattern::Slice {
                ...
                let start = *start;
                if start > 0 {
                    child = Arc::new(move |from| Box::new(child(from).skip(start)));
                }
                if let Some(length) = *length {
                    child = Arc::new(move |from| Box::new(child(from).take(length)));
                }
                child
            }
            ...
            GraphPattern::Group {
                ...
                Arc::new(move |from| {
                    ...
                    let key_variables = Arc::clone(&key_variables);
                    let mut errors = Vec::default();
            ...
            GraphPattern::Service {
                ...
                let variables = Arc::from(encoded_variables.as_slice());
                let eval = self.clone();
                Arc::new(move |from| {
        ...
        match expression {
            ...
            AggregateExpression::FunctionCall {
                ...
                AggregateFunction::GroupConcat { separator } => {
                    let separator = Arc::from(separator.as_deref().unwrap_or(" "));
                    let evaluator =
                        self.expression_evaluator(expr, encoded_variables, stat_children);
                    if *distinct {
                        Box::new(move || AccumulatorWrapper::DistinctExpression {
                            evaluator: Arc::clone(&evaluator),
                            seen: FxHashSet::default(),
                            accumulator: Some(Box::new(GroupConcatAccumulator::new(Arc::clone(
                                &separator,
                            )))),
                        })
                    } else {
    ...
    fn internal_expression_evaluator(
        ...
        encoded_variables: &mut Vec<Variable>,
        stat_children: &mut Vec<Arc<EvalNodeWithStats>>,
    ) -> Option<Arc<dyn Fn(&InternalTuple<D>) -> Option<D::InternalTerm>>> {
        Some(match expression {
            Expression::NamedNode(t) => {
                let t = self.encode_term(t.clone()).ok();
                Arc::new(move |_| t.clone())
            }
    ...
    fn effective_boolean_value_expression_evaluator(
        ...
        encoded_variables: &mut Vec<Variable>,
        stat_children: &mut Vec<Arc<EvalNodeWithStats>>,
    ) -> Arc<dyn Fn(&InternalTuple<D>) -> Option<bool>> {
        // TODO: avoid dyn?
        if let Some(eval) =
            self.internal_expression_evaluator(expression, encoded_variables, stat_children)
        {
            let dataset = self.dataset.clone();
            return Arc::new(move |tuple| {
                dataset
        ...
        Arc::new(move |tuple| eval(tuple)?.effective_boolean_value())
    }
    ...
    fn expression_evaluator(
        ...
        encoded_variables: &mut Vec<Variable>,
        stat_children: &mut Vec<Arc<EvalNodeWithStats>>,
    ) -> Arc<dyn Fn(&InternalTuple<D>) -> Option<ExpressionTerm>> {
        match expression {
            Expression::NamedNode(t) => {
            ...
            Expression::SameTerm(a, b) => {
                if let (Some(a), Some(b)) = (
                    ...
                    return Arc::new(move |tuple| Some((a(tuple)? == b(tuple)?).into()));
                };
                let a = self.expression_evaluator(a, encoded_variables, stat_children);
                let b = self.expression_evaluator(b, encoded_variables, stat_children);
            ...
            Expression::Not(e) => {
                ...
                Arc::new(move |tuple| Some((!e(tuple)?).into()))
            }
            Expression::Coalesce(l) => {
                let l: Vec<_> = l
                    .iter()
    ...
    ) -> Result<Arc<PropertyPath<D>>, QueryEvaluationError> {
...
impl<D: QueryDataset> Clone for SimpleEvaluator<D> {
    fn clone(&self) -> Self {
        Self {
            ...
            custom_functions: Arc::clone(&self.custom_functions),
...
fn decode_bindings<D: QueryDataset>(
    dataset: EvalDataset<D>,
...
fn encode_bindings<D: QueryDataset>(
    dataset: EvalDataset<D>,
    variables: Arc<[Variable]>,
...
fn put_variable_value<D: QueryDataset>(
    selector: &Variable,
    variables: &[Variable],
...
enum AccumulatorWrapper<D: QueryDataset> {
    CountTuple {
        count: u64,
    },
    CountDistinctTuple {
    ...
    CountInternal {
        evaluator: Arc<dyn Fn(&InternalTuple<D>) -> Option<D::InternalTerm>>,
        count: u64,
    },
    ...
    Sample {
        ...
        evaluator: Arc<dyn Fn(&InternalTuple<D>) -> Option<ExpressionTerm>>,
...
impl<D: QueryDataset> AccumulatorWrapper<D> {
...
enum TupleSelector<D: QueryDataset> {
    Constant(D::InternalTerm),
    Variable(usize),
    #[cfg(feature = "rdf-star")]
    TriplePattern(Arc<TripleTupleSelector<D>>),
}

impl<D: QueryDataset> TupleSelector<D> {
            ...
            GroundTermPattern::Triple(triple) => {
                match (
                    ...
                    (subject, predicate, object) => {
                        Self::TriplePattern(Arc::new(TripleTupleSelector {
                            subject,
                            predicate,
                            object,
                        }))
...
impl<D: QueryDataset> Clone for TupleSelector<D> {
    fn clone(&self) -> Self {
        match self {
            Self::Constant(c) => Self::Constant(c.clone()),
            Self::Variable(v) => Self::Variable(*v),
            #[cfg(feature = "rdf-star")]
            Self::TriplePattern(t) => Self::TriplePattern(Arc::clone(t)),
        }
    }
}

#[cfg(feature = "rdf-star")]
struct TripleTupleSelector<D: QueryDataset> {
...
#[cfg_attr(not(feature = "rdf-star"), allow(clippy::unnecessary_wraps))]
fn put_pattern_value<D: QueryDataset>(
...
pub fn are_compatible_and_not_disjointed<D: QueryDataset>(
    a: &InternalTuple<D>,
    b: &InternalTuple<D>,
) -> bool {
...
pub enum PropertyPath<D: QueryDataset> {
    Path(D::InternalTerm),
    Reverse(Arc<Self>),
    Sequence(Arc<Self>, Arc<Self>),
    Alternative(Arc<Self>, Arc<Self>),
    ZeroOrMore(Arc<Self>),
    OneOrMore(Arc<Self>),
    ZeroOrOne(Arc<Self>),
    NegatedPropertySet(Arc<[D::InternalTerm]>),
}

struct PathEvaluator<D: QueryDataset> {
    dataset: EvalDataset<D>,
}

impl<D: QueryDataset> PathEvaluator<D> {
...
impl<D: QueryDataset> Clone for PathEvaluator<D> {
    fn clone(&self) -> Self {
        Self {
            dataset: self.dataset.clone(),
        }
...
struct CartesianProductJoinIterator<D: QueryDataset> {
...
impl<D: QueryDataset> Iterator for CartesianProductJoinIterator<D> {
...
struct HashJoinIterator<D: QueryDataset> {
    probe_iter: Peekable<InternalTuplesIterator<D>>,
    built: InternalTupleSet<D>,
    buffered_results: Vec<Result<InternalTuple<D>, QueryEvaluationError>>,
}

impl<D: QueryDataset> Iterator for HashJoinIterator<D> {
    type Item = Result<InternalTuple<D>, QueryEvaluationError>;
...
struct HashLeftJoinIterator<D: QueryDataset> {
    left_iter: InternalTuplesIterator<D>,
    right: InternalTupleSet<D>,
    buffered_results: Vec<Result<InternalTuple<D>, QueryEvaluationError>>,
    expression: Arc<dyn Fn(&InternalTuple<D>) -> Option<bool>>,
}

impl<D: QueryDataset> Iterator for HashLeftJoinIterator<D> {
    type Item = Result<InternalTuple<D>, QueryEvaluationError>;
...
#[cfg(feature = "sep-0006")]
struct ForLoopLeftJoinIterator<D: QueryDataset> {
    right_evaluator: Arc<dyn Fn(InternalTuple<D>) -> InternalTuplesIterator<D>>,
    left_iter: InternalTuplesIterator<D>,
    current_right: InternalTuplesIterator<D>,
}

#[cfg(feature = "sep-0006")]
impl<D: QueryDataset> Iterator for ForLoopLeftJoinIterator<D> {
    type Item = Result<InternalTuple<D>, QueryEvaluationError>;
...
struct UnionIterator<D: QueryDataset> {
    plans: Vec<Arc<dyn Fn(InternalTuple<D>) -> InternalTuplesIterator<D>>>,
    input: InternalTuple<D>,
    current_iterator: InternalTuplesIterator<D>,
    current_plan: usize,
}

impl<D: QueryDataset> Iterator for UnionIterator<D> {
    type Item = Result<InternalTuple<D>, QueryEvaluationError>;
...
struct ConsecutiveDeduplication<D: QueryDataset> {
    inner: InternalTuplesIterator<D>,
    current: Option<InternalTuple<D>>,
}

impl<D: QueryDataset> Iterator for ConsecutiveDeduplication<D> {
    type Item = Result<InternalTuple<D>, QueryEvaluationError>;
...
struct ConstructIterator<D: QueryDataset> {
...
impl<D: QueryDataset> Iterator for ConstructIterator<D> {
...
fn get_triple_template_value<D: QueryDataset>(
...
struct DescribeIterator<D: QueryDataset> {
    eval: SimpleEvaluator<D>,
...
impl<D: QueryDataset> Iterator for DescribeIterator<D> {
    type Item = Result<Triple, QueryEvaluationError>;
...
fn error_evaluator<D: QueryDataset>(
    error: QueryEvaluationError,
) -> Arc<dyn Fn(InternalTuple<D>) -> InternalTuplesIterator<D>> {
    let e = Mutex::new(Some(error));
    Arc::new(move |_| {
        if let Some(e) = take(&mut *e.lock().unwrap()) {
            Box::new(once(Err(e)))
        } else {
            Box::new(empty())
        }
...
enum ComparatorFunction<D: QueryDataset> {
    Asc(Arc<dyn Fn(&InternalTuple<D>) -> Option<ExpressionTerm>>),
    Desc(Arc<dyn Fn(&InternalTuple<D>) -> Option<ExpressionTerm>>),
}

struct InternalTupleSet<D: QueryDataset> {
    key: Vec<usize>,
    map: FxHashMap<u64, Vec<InternalTuple<D>>>,
    len: usize,
}

impl<D: QueryDataset> InternalTupleSet<D> {
...
impl<D: QueryDataset> Extend<InternalTuple<D>> for InternalTupleSet<D> {
...
struct StatsIterator<D: QueryDataset> {
    inner: InternalTuplesIterator<D>,
    stats: Arc<EvalNodeWithStats>,
}

impl<D: QueryDataset> Iterator for StatsIterator<D> {
    type Item = Result<InternalTuple<D>, QueryEvaluationError>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        {
            let mut exec_duration = match self.stats.exec_duration.lock() {
                Ok(exec_duration) => exec_duration,
                Err(e) => return Some(Err(QueryEvaluationError::Dataset(e.to_string().into()))),
            };
            *exec_duration = exec_duration.and_then(|d| d.checked_add(start.elapsed()?));
        }
        if matches!(result, Some(Ok(_))) {
            self.stats.exec_count.fetch_add(1, AtomicOrdering::Relaxed);
        }
        result
    }
}
...
pub struct EvalNodeWithStats {
    pub label: String,
    pub children: Vec<Arc<EvalNodeWithStats>>,
    pub exec_count: AtomicUsize,
    pub exec_duration: Mutex<Option<DayTimeDuration>>,
}
...
impl fmt::Debug for EvalNodeWithStats {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(exec_duration) = *self.exec_duration.lock().map_err(|_| fmt::Error)? {
            obj.field(
                "number of results",
                &self.exec_count.load(AtomicOrdering::Relaxed),
            );
            obj.field(
                "duration in seconds",
