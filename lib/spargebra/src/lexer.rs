...
pub fn lex_sparql(slice: &str) -> Result<Vec<Spanned<Token<'_>>>, Vec<Rich<'_, char>>> {
...
fn build_lexer<'src>()
-> Boxed<'src, 'src, &'src str, Vec<Spanned<Token<'src>>>, extra::Err<Rich<'src, char, SimpleSpan>>>
{
    // [193]   	PN_LOCAL_ESC 	  ::=   	'\' ( '_' | '~' | '.' | '-' | '!' | '$' | '&' | "'" | '(' | ')' | '*' | '+' | ',' | ';' | '=' | '/' | '?' | '#' | '@' | '%' )
    let pn_local_esc = just('\\').then(select! {
        '_', '~', '.', '-', '!', '$', '&', '\'', '(', ')', '*', '+', ',', ';', '=', '/', '?', '#',
        '@', '%'
    });

    // [192]   	HEX 	  ::=   	[0-9] | [A-F] | [a-f]
    let hex = select! {'0'..='9' | 'A'..='F' | 'a'..='f' };

    // [191]   	PERCENT 	  ::=   	'%' HEX HEX
    let percent = just('%').then(hex).then(hex);
    ...
    let pn_chars_u = pn_chars_base.or(select! { '_' });

    // [187]   	PN_CHARS 	  ::=   	PN_CHARS_U | '-' | [0-9] | #x00B7 | [#x0300-#x036F] | [#x203F-#x2040]
    let pn_chars = pn_chars_u.or(
        select! { '-' |'0'..='9' | '\u{00B7}'|'\u{0300}'..='\u{036F}'|'\u{203F}'..='\u{2040}' },
    );

    // [189]   	PN_LOCAL 	  ::=   	(PN_CHARS_U | ':' | [0-9] | PLX ) ((PN_CHARS | '.' | ':' | PLX)* (PN_CHARS | ':' | PLX) )?
    let pn_local_element = pn_chars.or(select! { '.' | ':' }).or(plx);
    let pn_local = pn_chars_u
        .or(select! { ':' | '0'..='9' })
        .or(plx)
        .then(
            pn_local_element
                .or(just('.').repeated().then(pn_local_element).ignored())
                .repeated(),
        )
        .to_slice();
    ...
    let pn_prefix = pn_chars_base.then(
        pn_chars
            .or(just('.').repeated().then(pn_chars).ignored())
            .repeated(),
    );
    ...
    let varname = pn_chars_u
        ...
        .then(
            pn_chars_u
                .or(select! { '0'..='9' |'\u{00B7}' | '\u{0300}'..='\u{036F}'| '\u{203F}'..='\u{2040}'})
                .repeated(),
        )
        .to_slice();
    ...
    let uchar = just("\\u")
        .then(hex.repeated().exactly(4))
        .or(just("\\U").then(hex.repeated().exactly(8)))
        .ignored();
    ...
    let string_literal_long2_char = none_of(['"', '\\']).ignored().or(echar.ignored());
    #[cfg(not(feature = "standard-unicode-escaping"))]
    let string_literal_long2_char = string_literal_long2_char.or(uchar.ignored());
    let string_literal_long2 = just("\"\"\"")
        .ignore_then(
            just('"')
                .then(just('"').or_not())
                .or_not()
                .then(string_literal_long2_char)
                .repeated()
                .to_slice(),
        )
        .then_ignore(just("\"\"\""))
    ...
    let string_literal_long1_char = none_of(['\'', '\\']).ignored().or(echar.ignored());
    #[cfg(not(feature = "standard-unicode-escaping"))]
    let string_literal_long1_char = string_literal_long1_char.or(uchar.ignored());
    let string_literal_long1 = just("'''")
        .ignore_then(
            just('\'')
                .then(just('\'').or_not())
                .or_not()
                .then(string_literal_long1_char)
                .repeated()
                .to_slice(),
        )
        .then_ignore(just("'''"))
    ...
    let string_literal2_char = none_of(['\x22', '\x5C', '\x0A', '\x0D'])
        .ignored()
        .or(echar.ignored());
    #[cfg(not(feature = "standard-unicode-escaping"))]
    let string_literal2_char = string_literal2_char.or(uchar.ignored());
    let string_literal2 = just('"')
        .ignore_then(string_literal2_char.repeated().to_slice())
        .then_ignore(just('"'))
        .map(Token::StringLiteral2);

    // [176]   	STRING_LITERAL1 	  ::=   	"'" ( ([^#x27#x5C#xA#xD]) | ECHAR )* "'"
    let string_literal1_char = none_of(['\x27', '\x5C', '\x0A', '\x0D'])
        .ignored()
        .or(echar.ignored());
    #[cfg(not(feature = "standard-unicode-escaping"))]
    let string_literal1_char = string_literal1_char.or(uchar.ignored());
    let string_literal1 = just('\'')
        .ignore_then(string_literal1_char.repeated().to_slice())
        .then_ignore(just('\''))
    ...
    // [164]   	VAR2 	  ::=   	'$' VARNAME
    let var2 = just('$').ignore_then(varname).map(Token::Var2);
    ...
    // [159]   	IRIREF 	  ::=   	'<' ([^<>"{}|^`\]-[#x00-#x20])* '>'
    let iri_ref_char = any().and_is(
        select! { '<' | '>' | '"' | '{' | '}' | '|' | '^' | '`' | '\'' | '\x00' ..= '\x20' }.not(),
    );
    #[cfg(not(feature = "standard-unicode-escaping"))]
    let iri_ref_char = iri_ref_char.ignored().or(uchar);
    let iri_ref = just('<')
        .ignore_then(iri_ref_char.repeated().to_slice())
        .then_ignore(just('>'))
        .map(Token::IriRef);
    ...
    let token_or_comment = token.spanned().map(Some).or(comment.map(|_| None));
