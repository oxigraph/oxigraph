...
use crate::{GraphUpdateOperation, Update, ast};
...
impl<'a> AlgebraBuilder<'a> {
    ...
    ) -> Result<Query, AlgebraBuilderError> {
        Ok(Query::Select {
        ...
        Ok(Query::Construct {
            ...
            pattern: self.build_select(
                ast::SelectClause {
                    ...
                    bindings: SimpleSpan::new((), 0..0).make_wrapped(Vec::new()),
                },
                where_clause,
                query.solution_modifier,
                values_clause,
    ...
    fn build_describe_query(
        ...
        let mut pattern = self.build_select(
            ast::SelectClause {
                option: ast::SelectionOption::Default,
                bindings: query.targets.span.make_wrapped(
                    query
                        .targets
                        .inner
                        .iter()
        ...
        // We add the IRIS
        for target in query.targets.inner {
    ...
    fn build_select(
        ...
        let select_expressions = select_clause
            .bindings
            .inner
            .into_iter()
            .map(|binding| {
        ...
        let (name, expression, distinct) = match aggregate.inner {
            ...
            ast::Aggregate::GroupConcat(distinct, expression, separator) => (
                AggregateFunction::GroupConcat {
                    separator: separator.map(|s| Self::build_string(s)),
                },
                expression,
                distinct,
            ),
    ...
    fn build_string(string: ast::String<'_>) -> String {
        unescape_string(string.0)
    }
    ...
    pub fn build_update(mut self, update: ast::Update<'_>) -> Result<Update, AlgebraBuilderError> {
        ...
        for (prologue, update1) in update.0 {
            self.apply_prologue(prologue)?;
            if let Some(update1) = update1 {
                match update1 {
                    ast::Update1::Load { silent, from, to } => {
                        operations.push(GraphUpdateOperation::Load {
                            silent,
                    ...
                    ast::Update1::Clear { silent, graph } => {
                        operations.push(GraphUpdateOperation::Clear {
                            silent,
                            graph: self.build_graph_target(graph)?,
                        })
                    }
                    ast::Update1::Drop { silent, graph } => {
                        operations.push(GraphUpdateOperation::Drop {
                            silent,
                            graph: self.build_graph_target(graph)?,
                        })
                    }
                    ast::Update1::Create { silent, graph } => {
                        operations.push(GraphUpdateOperation::Create {
                            silent,
                    ...
                    ast::Update1::Add { from, to, .. } => {
                        ...
                        operations.push(copy_graph(from, to))
                    }
                    ...
                    ast::Update1::DeleteWhere { pattern } => {
                        ...
                        operations.push(GraphUpdateOperation::DeleteInsert {
                            delete,
                            insert: Vec::new(),
                            using: None,
                    ...
                    ast::Update1::InsertData { quads } => {
                        operations.push(GraphUpdateOperation::InsertData {
                            data: self.build_quads(quads)?,
                        })
                    }
                    ast::Update1::DeleteData { quads } => {
                        operations.push(GraphUpdateOperation::DeleteData {
...
#[cfg(not(feature = "standard-unicode-escaping"))]
fn unescape_iriref(mut input: &str) -> String {
...
fn unescape_string(mut input: &str) -> String {
    let mut output = String::with_capacity(input.len());
    while let Some((before, after)) = input.split_once('\\') {
        output.push_str(before);
        let mut after = after.chars();
...
#[cfg(not(feature = "standard-unicode-escaping"))]
fn read_hex_char<const SIZE: usize>(input: &str) -> (char, &str) {
    if let Some(escape) = input.get(..SIZE) {
            ...
            .and_then(char::from_u32)
        {
            ...
            unreachable!("\\u escape sequence should be followed by hexadecimal digits")
        }
    } else {
        unreachable!("\\u escape sequence should be followed by hexadecimal digits")
    }
