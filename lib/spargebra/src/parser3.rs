...
-> impl Parser<'src, I, Query<'src>, extra::Err<Rich<'src, Token<'src>, SimpleSpan>>> {
    ...
    expression.define(
        primary_expression
            ...
            .pratt((
                ...
                infix(left(3), operator("!="), |l, _, r, _| {
                    Expression::Not(Box::new(Expression::NotEqual(Box::new(l), Box::new(r))))
                }),
                infix(left(3), operator("<"), |l, _, r, _| {
    ...
    let inline_data = keyword("VALUES")
        ...
        .map(|(variables, values)| GraphPatternElement::Values { variables, values });
    ...
    let graph_pattern_not_triples = group_or_union_graph_pattern
        ...
        .or(inline_data);
    #[cfg(feature = "sep-0006")]
    let graph_pattern_not_triples = graph_pattern_not_triples.or(lateral_graph_pattern);
