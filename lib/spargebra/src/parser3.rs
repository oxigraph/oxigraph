...
fn build_parsers<'src, I: ValueInput<'src, Token = Token<'src>, Span = SimpleSpan>>() -> (
    ...
    let iriref = select! { Token::IriRef(i) => IriRef(i) }
        .spanned()
        .labelled("an iri");
    ...
    let rdf_literal = string
        ...
        .map(
            |(string, extra): (String<'_>, Option<Spanned<Either<_, _>>>)| {
    ...
    triple_term_data.define(
        iri.then(
            iri.map(IriOrA::Iri)
                .or(case_sensitive_keyword("a").map(|()| IriOrA::A)),
        )
    ...
    let path = recursive(|path| {
        ...
        let path_one_in_property_set = choice((
            ...
            case_sensitive_keyword("a").map(|()| PathOneInPropertySet::A),
            operator("^").ignore_then(iri.map(PathOneInPropertySet::InverseIri)),
            operator("^")
                .ignore_then(case_sensitive_keyword("a").map(|()| PathOneInPropertySet::InverseA)),
        ));
        ...
        let path_primary = choice((
            iri.map(Path::Iri),
            case_sensitive_keyword("a").map(|()| Path::A),
    ...
    let data_block_value = choice((
        ...
        keyword("UNDEF").map(|()| DataBlockValue::Undef),
        #[cfg(feature = "sparql-12")]
    ...
    let graph_ref_all = choice((
        ...
        keyword("DEFAULT").map(|()| GraphRefAll::Default),
        keyword("NAMED").map(|()| GraphRefAll::Named),
        keyword("ALL").map(|()| GraphRefAll::All),
    ));

    // [47]   	GraphOrDefault 	  ::=   	'DEFAULT' | 'GRAPH'? iri
    let graph_or_default = keyword("DEFAULT")
        .map(|()| GraphOrDefault::Default)
        .or(keyword("GRAPH")
            .or_not()
            .ignore_then(iri)
    ...
    // or Update 	  ::=   	Prologue (Update1)? ( ';' Prologue (Update1)? )*
    let update = prologue
        .clone()
        .then(update1.or_not())
        .separated_by(operator(";"))
