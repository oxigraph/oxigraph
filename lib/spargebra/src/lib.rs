...
pub use crate::error::{SparqlSyntaxError2, TextPosition};
use crate::lexer::lex_sparql;
use crate::parser3::{parse_sparql_query, parse_sparql_update};
...
impl SparqlParser2 {
    ...
    pub fn parse_query(&self, query: &str) -> Result<Query, SparqlSyntaxError2> {
        #[cfg(feature = "standard-unicode-escaping")]
        let query = parser::unescape_unicode_codepoints(query);
        let tokens = lex_sparql(&query).map_err(|e| SparqlSyntaxError2::from_chumsky(e, &query))?;
        let ast = parse_sparql_query(&tokens, query.len())
            .map_err(|e| SparqlSyntaxError2::from_chumsky(e, &query))?;
        AlgebraBuilder::new(
            self.base_iri.clone(),
            self.prefixes.clone(),
        ...
        .build_query(ast)
        .map_err(|e| SparqlSyntaxError2::from_algebra_builder(e, &query))
    }
    ...
    pub fn parse_update(&self, update: &str) -> Result<Update, SparqlSyntaxError2> {
        #[cfg(feature = "standard-unicode-escaping")]
        let update = parser::unescape_unicode_codepoints(update);
        let tokens =
            lex_sparql(&update).map_err(|e| SparqlSyntaxError2::from_chumsky(e, &update))?;
        let ast = parse_sparql_update(&tokens, update.len())
            .map_err(|e| SparqlSyntaxError2::from_chumsky(e, &update))?;
        AlgebraBuilder::new(
            self.base_iri.clone(),
            self.prefixes.clone(),
            &self.custom_aggregate_functions,
        ...
        .build_update(ast)
        .map_err(|e| SparqlSyntaxError2::from_algebra_builder(e, &update))
    }
}
