...
/// A position in a text i.e. a `line` number starting from 0, a `column` number starting from 0 (in number of bytes) and a global file `offset` starting from 0 (in number of bytes).
