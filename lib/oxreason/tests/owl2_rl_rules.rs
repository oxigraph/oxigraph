...
fn prp_trp_transitive_property_closure() {
    ...
    assert!(
        report.added >= 3,
        "expected at least 3 inferred edges, got {}",
        report.added
    );
...
fn cax_dw_disjoint_classes_raise_inconsistent_error() {
    ...
    match err {
        ReasonError::Inconsistent {
            ...
            assert!(
                individual.contains("Acme"),
                "unexpected individual {individual}"
            );
...
fn prp_spo2_chains_grandparent_from_parent_of_parent() {
    ...
    assert!(g.contains(&Triple::new(ex("Alice"), ex("hasGrandparent"), ex("Carol"),)));
}
...
fn prp_spo2_chains_uncle_from_parent_then_sibling() {
    ...
    assert!(g.contains(&Triple::new(ex("Alice"), ex("hasUncle"), ex("Charlie"),)));
}
...
fn prp_spo2_does_not_fire_when_chain_breaks() {
    ...
    let ok = g
        ...
        .any(|t| t.subject == oxrdf::NamedOrBlankNodeRef::NamedNode(ex("Alice").as_ref()));
    assert!(
        !ok,
        "hasGrandparent must not be materialised for a broken chain"
    );
}
...
fn expand_streaming_emits_only_new_inferences() {
    ...
    let report = Reasoner::new(ReasonerConfig::owl2_rl())
        .expand_streaming(
            &mut g,
            |triple: &Triple| -> Result<(), std::convert::Infallible> {
    ...
    assert!(
        sunk.contains(&expected),
        "sink must see the cax-sco inference"
    );
    assert!(
        !sunk.contains(&Triple::new(
            ex("Company"),
            rdfs::SUB_CLASS_OF,
            ex("LegalPerson")
        )),
...
fn expand_streaming_counts_match_report_added() {
    ...
    let report = Reasoner::new(ReasonerConfig::owl2_rl())
        .expand_streaming(
            &mut g,
            |_: &Triple| -> Result<(), std::convert::Infallible> {
                count += 1;
                Ok(())
            },
        )
    ...
    assert!(
        count >= 3,
        "prp-trp must close at least 3 edges over a chain of four"
    );
}
