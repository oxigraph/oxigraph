//! Public reasoner API.
//!
//! [`Reasoner`] is the entry point. Construct it with a [`ReasonerConfig`]
//! and apply OWL 2 RL forward chaining over an [`oxrdf::Graph`] via
//! [`Reasoner::expand`] (in place), [`Reasoner::expand_into`] (clone and
//! return a new graph), or the streaming variants
//! [`Reasoner::expand_streaming`] and [`Reasoner::expand_streaming_from`]
...
impl Reasoner {
    ...
    pub fn expand(&self, graph: &mut Graph) -> Result<ReasoningReport, ReasonError> {
        match self.expand_streaming(graph, |_: &Triple| -> Result<(), Infallible> { Ok(()) }) {
            Ok(report) => Ok(report),
...
fn map_run_result<E>(
    ...
    match run {
        ...
        Err(engine::ExpandError::Inconsistency(other)) => {
            Err(ReasonStreamError::Reason(ReasonError::InconsistentAxiom {
...
mod tests {
    ...
    fn expand_on_empty_graph_returns_zero_added() {
        ...
        let report = reasoner
            .expand(&mut g)
            .expect("empty graph must reason cleanly");
