use crate::sparql::datafusion::functions::{EffectiveBooleanValue, OrderByCollation, ToRdfLiteral};
...
impl<'a> SparqlPlanBuilder<'a> {
        ...
        match pattern {
            ...
            GraphPattern::OrderBy { inner, expression } => {
                ...
                let sorts = expression
                    .iter()
                    .map(|e| self.order_expression(e, plan.schema(), external_schema))
                    .collect::<Result<Vec<_>>>()?;
                plan.sort(sorts)
            }
    ...
    fn order_expression(
        ...
        Ok(match expression {
            OrderExpression::Asc(e) => self
                .order_by_collation(self.expression(e, schema, external_schema)?)
                .sort(true, true),
            OrderExpression::Desc(e) => self
                .order_by_collation(self.expression(e, schema, external_schema)?)
                .sort(false, true),
        })
    }
    ...
    fn order_by_collation(&self, expr: Expr) -> Expr {
        ScalarUDF::new_from_impl(OrderByCollation::new(Arc::clone(&self.dataset))).call(vec![expr])
    }
}
