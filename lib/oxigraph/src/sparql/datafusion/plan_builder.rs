use crate::sparql::datafusion::function::{
    effective_boolean_value, greater_than, greater_than_or_equal, lang, lang_matches, less_than,
    less_than_or_equal, order_by_collation, plus, regex, str, sum, term_equals, to_rdf_literal,
    xsd_double,
};
...
impl<'a> SparqlPlanBuilder<'a> {
                    ...
                    .map(|(target_var, expression)| {
                        let (aggregate, convert_back_to_term) = match expression {
                            ...
                            AggregateExpression::FunctionCall {
                                ...
                                match (name, distinct) {
                                    (AggregateFunction::Count, false) => (count(expression), true),
                                    (AggregateFunction::Count, true) => {
                                        (count_distinct(expression), true)
                                    }
                                    (AggregateFunction::Sum, distinct) => (
                                        sum(Arc::clone(&self.dataset), expression, *distinct),
                                        false,
                                    ),
                                    _ => {
    ...
    fn expression(
        ...
        Ok(match expression {
            ...
            _ => {
                return not_impl_err!("All expressions are not implemented yet");
            }
        })
    }
