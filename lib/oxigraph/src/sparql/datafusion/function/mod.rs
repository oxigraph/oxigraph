...
pub fn sum(dataset: Arc<DatasetView<'static>>, input: Expr, distinct: bool) -> Expr {
    ...
    impl TermAccumulator for SumAccumulator {
        ...
        fn evaluate(self) -> Option<ExpressionTerm> {
            self.sum
        }
        ...
        fn merge(&mut self, state: Vec<Option<ExpressionTerm>>) {
            self.sum = self
                .sum
                .take()
                .and_then(|sum| do_add(sum, state.into_iter().next()??));
        }
    }
