...
use datafusion::common::{Constraint, Constraints, ScalarValue, Statistics};
...
use datafusion::physical_plan::projection::{
    ProjectionExpr, all_alias_free_columns, new_projections_for_columns,
};
use datafusion::physical_plan::{
    ColumnStatistics, DisplayFormatType, ExecutionPlan, RecordBatchStream,
};
use futures::Stream;
...
impl DataSource for QuadDataSource {
    ...
    fn statistics(&self) -> Result<Statistics> {
        ...
        let mut stats = Statistics::new_unknown(&self.schema)
            ...
            .with_total_byte_size(Precision::Inexact(size * 17 * 3)); // TODO: graph name
        for _ in self.schema.fields() {
            stats = stats.add_column_statistics(
                ColumnStatistics::new_unknown().with_null_count(Precision::Exact(0)),
            );
        }
        Ok(stats)
    }
