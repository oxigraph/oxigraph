...
impl DatafusionEvaluator {
            ...
            match query {
                Query::Select { pattern, .. } => {
                    let Some((variables, stream)) = self
                        .execute_graph_pattern(pattern, None, Arc::clone(&dataset))
                        .await?
                    else {
                        return Ok(None);
                    };
                ...
                Query::Ask { pattern, .. } => {
                    let Some((_, mut stream)) = self
                        .execute_graph_pattern(pattern, Some(1), Arc::clone(&dataset))
                        .await?
                    else {
                        return Ok(None);
                    };
    ...
    async fn execute_graph_pattern(
        ...
        let plan = match SparqlPlanBuilder::new(dataset)
            .plan_for_graph_pattern(pattern, &HashMap::new())
            .and_then(|mut plan| {
    ...
    pub fn explain(
        ...
        let logical_plan = match SparqlPlanBuilder::new(dataset)
            .plan_for_graph_pattern(pattern, &HashMap::new())
            .and_then(|mut plan| {
                if let Some(limit) = limit {
                    plan = plan.limit(0, Some(limit))?;
